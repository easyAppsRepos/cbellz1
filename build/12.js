                ...
                _this.apiProvider.getCentrosMapa({ lat: _this.myPosition.latitude, lon: _this.myPosition.longitude })
                    ...
                    data.forEach(function (element, index) {
                        ...
                        frame.innerHTML = ["<span style='color: #2FD99B;font-size: 17px;'><b>" + element.nombre + "</b></span><br>",
                             true ? element.rate.toFixed(1) : 0 + " >= 1}\" name=\"star\"></ion-icon>\n        <ion-icon  [ngClass]=\"{'ratingStar': " + element.rate ? element.rate.toFixed(1) : 0 + ">= 2}\" name=\"star\"></ion-icon>\n        <ion-icon [ngClass]=\"{'ratingStar': " + element.rate ? element.rate.toFixed(1) : 0 + ">= 3}\"  name=\"star\"></ion-icon>\n        <ion-icon  [ngClass]=\"{'ratingStar': " + element.rate ? element.rate.toFixed(1) : 0 + ">= 4}\" name=\"star\"></ion-icon>\n        <ion-icon [ngClass]=\"{'ratingStar': " + element.rate ? element.rate.toFixed(1) : 0 + ">= 5}\" name=\"star\"></ion-icon>\n        </span>",
                            "<span style='color: #888;font-size: 17px;  '><ion-icon name='ios-pin' role='img' style='  margin-right: 8px;      vertical-align: middle;   font-size: 21px;color:#2FD99B;' class='icon icon-ios ion-ios-pin' aria-label='pin' ng-reflect-name='ios-pin'></ion-icon>A " + (element.distance ? element.distance.toFixed(1) : '-') + "Km aprox.</span><br>",
                            "<button >Mas Informacion</button>"].join("");
